...
def what_conclusion(parsed_temperature):
    try:
        ...
        if temperature < 18:  # Если (if) температура строго меньше 18:
            return 'оч холодно'  # вернуть (return) фразу со словом 'холодно'
        elif 18 <= temperature <= 27:  # Если температура в диапазоне от 18 до 27 включительно
            return 'в самый раз'  # вернуть фразу со словами 'в самый раз'
        else:  # В остальных случаях:
            return 'жарко'  # вернуть фразу со словом 'жарко'

    except ValueError:
        # if int(parsed_temperature):# Если parsed_temperature не удалось преобразовать в число —
        return 'Не могу узнать погоду...'  # значит, погодный сервис сломался и надо вернуть фразу "Не могу узнать погоду..."
